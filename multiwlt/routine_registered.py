...
def plan_reference(ds):
    ...
    if len(refs)!=1 or not getattr(refs[0],'ReferencedSOPInstanceUID',None):
        raise ValueError('Genau eine RTPLAN-Referenz erforderlich.')
...
def record_evidence(folder, plan, expected_ref, beam_numbers):
    ...
    if not folder:raise ValueError('Andere Plan-ID: RTPLAN-Rückexport binden oder Bestrahlungsprotokolle auswählen.')
    ...
    for path in sorted(Path(folder).glob('*.dcm')):
        ...
        for session in ds.TreatmentSessionBeamSequence:
            ...
            if n in found:raise ValueError('Mehrere Bestrahlungsprotokolle zum selben Feld; Sitzung eingrenzen.')
            beam=source[n];cp=beam.ControlPointSequence[0]
            if str(session.BeamName)!=str(beam.BeamName) or str(session.BeamType)!='STATIC':raise ValueError('Feldname/-typ passt nicht zum Referenzplan.')
            if str(session.TreatmentTerminationStatus)!='NORMAL':raise ValueError('Bestrahlung nicht normal abgeschlossen.')
            if str(ds.TreatmentMachineSequence[0].TreatmentMachineName)!=str(beam.TreatmentMachineName):raise ValueError('Gerät im Protokoll passt nicht.')
            ...
            for delivered in session.ControlPointDeliverySequence:
                ...
                if planned.keys()!=devices.keys():raise ValueError('MLC-/Backentypen unterscheiden sich.')
                for kind in planned:
                    if planned[kind].shape!=devices[kind].shape:raise ValueError('Blattanzahl unterscheidet sich.')
                    error=float(np.max(np.abs(planned[kind]-devices[kind])))
                    if not np.isfinite(error) or error>.05:raise ValueError('Protokollierte Öffnung weicht mehr als 0,05 mm vom Referenzplan ab.')
                    maximum=max(maximum,error)
                for key in ['GantryAngle','BeamLimitingDeviceAngle']:
                    if hasattr(delivered,key) and angle_delta(float(getattr(delivered,key)),float(getattr(cp,key)))>.1:raise ValueError('Protokollwinkel passt nicht.')
                if hasattr(delivered,'NominalBeamEnergy') and abs(float(delivered.NominalBeamEnergy)-float(cp.NominalBeamEnergy))>.01:raise ValueError('Energie passt nicht.')
            found[n]={'record_sha256':sha256(path),'max_device_difference_mm':maximum}
    if set(found)!=set(beam_numbers):raise ValueError('Passende Bestrahlungsprotokolle für alle Felder erforderlich.')
    return found
...
def measure_local(pixels,geometry,field_panel,collimator,size,diameter):
    ...
    if np.any(ij<0) or np.any(ij[...,0]>pixels.shape[0]-1) or np.any(ij[...,1]>pixels.shape[1]-1):raise ValueError('Suchfenster außerhalb des Detektors.')
    patch=ndimage.map_coordinates(pixels,[ij[...,0],ij[...,1]],order=1)
    edge,size_found,spread=_field_edges(patch,axis,axis,*size)
    if np.linalg.norm(edge)>.7:raise ValueError('Pylinac-Flächenmitte und Kantenmitte widersprechen sich (>0,7 mm).')
    ...
    if len(bb)!=1:raise ValueError('Keine eindeutige Kugel.')
    ...
    if clearance<.7:raise ValueError('Kugel zu nah am Feldrand.')
...
def analyse_registered_folder(image_folder,profile_path,output_root,*,positioned_in_ct=False,records_folder=None):
    if not positioned_in_ct:raise ValueError('Bildgeführte Positionierung in die CT-Sollposition muss bestätigt sein.')
    ref,beams,plan=load_profile(profile_path)
    if len(ref['balls'])!=3:raise ValueError('Dieser Detektor unterstützt drei Kugeln pro Bild.')
    datasets=[]
    for path in sorted(Path(image_folder).glob('*.dcm')):
        ds=pydicom.dcmread(path)
        if str(getattr(ds,'Modality',''))!='RTIMAGE':raise ValueError('Nur RTIMAGE-Dateien im MV-Ordner erwartet.')
        datasets.append((path,ds,sha256(path)))
    if len(datasets)!=len(beams):raise ValueError('Genau ein MV-Bild je Referenzfeld erforderlich.')
    refs={plan_reference(d) for _,d,_ in datasets}
    if len(refs)!=1:raise ValueError('Bilder referenzieren unterschiedliche Pläne.')
    actual_ref=refs.pop();numbers=[int(d.ReferencedBeamNumber) for _,d,_ in datasets]
    if len(set(numbers))!=len(numbers) or set(numbers)!={b['number'] for b in beams}:raise ValueError('Feldnummern fehlen oder sind doppelt.')
    evidence={} if actual_ref==ref['qa_plan_sop_hash'] else record_evidence(records_folder,plan,actual_ref,numbers)
    if any(not hasattr(d,k) for _,d,_ in datasets for k in ['PatientSupportAngle','TableTopPitchAngle','TableTopRollAngle']):raise ValueError('Tischwinkel müssen vollständig dokumentiert sein.')
    ...
    if any(angle_delta(a,b)>.05 for pose in poses for a,b in zip(pose,poses[0])):raise ValueError('Tischposition zwischen Bildern geändert; keine gemeinsame Positionierung.')
    ...
    result={'schema_version':1,'mode':'ct_positioned_multiwlt','status':'RESEARCH_EVALUATION',
        ...
        'reference_note':'Sollgeometrie aus CT und vorbereitetem R6-Plan; Bestrahlungsprotokolle stützen die Zuordnung. RTPLAN-Rückexport noch offen.' if evidence else 'Bildreferenzen stimmen mit dem geometrisch geprüften Referenzprofil überein.',
        'positioning':'Vom Anwender bestätigte bildgeführte CT-Sollposition; Tischwerte sind Setup-Korrektur, keine zusätzlich anzuwendende Restrotation.',
        'no_pose_fit_to_mv':True,'synthetic':False,'profile_sha256':sha256(profile_path),'plan_sha256':ref['qa_plan_sha256'],
        'record_evidence':evidence,'clinical_assessment':'Keine klinische Grenzwertbewertung; Messunsicherheit noch zu validieren.',
    ...
    for path,ds,digest in sorted(datasets,key=lambda item:int(item[1].ReferencedBeamNumber)):
        ...
        if angle_delta(g,beam['gantry_deg'])>.1 or angle_delta(c,beam['collimator_deg'])>.1:raise ValueError('Bildwinkel passt nicht zum Referenzfeld.')
        ...
        if not np.isclose(*geometry['pixel_iso_mm'],rtol=1e-5):raise ValueError('Dieser Detektor benötigt quadratische Pixel.')
        ...
        if max(costs[ii,jj])>3:raise ValueError('Teilfelder passen nicht eindeutig zum Referenzplan (3 mm Suchgrenze).')
        if any(sum(row<3)!=1 for row in costs):raise ValueError('Mehrdeutige Teilfeldzuordnung.')
        ...
        if sha256(path)!=digest:raise ValueError('Quelldatei während der Auswertung geändert.')
    ...
    plot_spheres(result['spheres'],output/'Strahlungs_Huellkugeln.png')
    ...
    with (output/'results.csv').open('w',encoding='utf-8-sig',newline='') as stream:
        w=csv.writer(stream,delimiter=';');w.writerow(['Gantry','BB','Soll_X_mm','Soll_Y_mm','Ist_X_mm','Ist_Y_mm','Extra_X_mm','Extra_Y_mm','Extra_Betrag_mm'])
